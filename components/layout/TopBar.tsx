import Link from 'next/link';
import Image from 'next/image';

export function TopBar() {
  return (
    <header className="sticky top-0 z-40 bg-card border-b border-border pt-safe-top">
      <div className="flex h-11 items-center justify-between px-4">
      <Link href="/" className="flex items-center gap-2 hover:opacity-75 transition-opacity">
        <Image src="/favicon.png" alt="Conexión Sur" width={28} height={28} className="rounded-full" />
        <span className="text-sm font-bold text-primary tracking-wide">Conexión Sur</span>
      </Link>
      <Link href="/buscar" className="flex h-8 w-8 items-center justify-center rounded-full text-secondary hover:text-primary">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/>
        </svg>
      </Link>
      </div>
    </header>
  );
}
